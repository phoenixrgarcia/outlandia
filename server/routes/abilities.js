...
async function getSecondTakeRemaining(characterId, round) {
  if (characterId !== "street_quintina_quintius") {
    return 0;
  }

  const used = await EventLog.countDocuments({
    action: "ability.second_take.rerolled",
...
function resolvePickpocketRoll({ character, targets, isReroll = false, remainingRerolls = 0 }) {
  const roll = { die: 20, value: rollDie(20) };
  const payoutRoll = roll.value >= 10 ? { die: 6, value: rollDie(6) } : null;
  const canUseSecondTake = (
    character.characterId === "street_quintina_quintius" &&
    roll.value < 10 &&
    remainingRerolls > 0
  );
  const suspicionChanges = roll.value < 10 && !canUseSecondTake
  ...
  const outcome = roll.value >= 16
    ...
    : canUseSecondTake
    ? `Failed. Second Take available: ${remainingRerolls} reroll${remainingRerolls === 1 ? "" : "s"} left this round.`
  ...
  return resultWithDefaults({
    ...
    targetMessages: notified
      ...
      : new Map(),
    message: `Pickpocket ${isReroll ? "reroll " : ""}d20: ${roll.value}. ${outcome} A GM has been notified.`,
    gmExtraLines: [
      ...(payoutRoll ? [`Payout roll: ${payoutRoll.value} on a d6. Result: ${payout}.`] : []),
      ...(canUseSecondTake ? ["Second Take reroll is available. Hold suspicion until the reroll is resolved or declined."] : []),
    ],
    canReroll: canUseSecondTake,
    rerollsRemaining: remainingRerolls,
  });
}
...
function chaosSurgeEffect(roll) {
  return roll >= 4 ? "Double next bribe/interrogation reward" : "+2 to next bribe/interrogation reward";
}
...
function resolveAbilityUse({ character, ability, handler, targets, secondTakeRemaining = 0 }) {
  const name = ability?.name || "";
  ...
  if (name === "Pickpocket") {
    return resolvePickpocketRoll({
      character,
      targets,
      remainingRerolls: secondTakeRemaining,
    });
  }
...
router.post("/:abilityType/use", requireAuth, ensureDatabase, async (req, res, next) => {
  try {
    ...
    if (abilityType === "passive" && !["Pity Trade", "Slip Through"].includes(ability.name)) {
      return res.status(400).json({
        error: "This passive ability is resolved in person with a GM.",
      });
    }
    ...
    const secondTakeRemaining = await getSecondTakeRemaining(character.characterId, round);

    const resolution = resolveAbilityUse({
      character,
      ability,
      handler,
      targets,
    ...
    await EventLog.create({
      ...
      details: {
        ...
        targetRolls: resolution.targetRolls,
        suspicionChanges: resolution.suspicionChanges,
        canReroll: resolution.canReroll,
        rerollsRemaining: resolution.rerollsRemaining,
      },
    });
...
router.post("/pickpocket/reroll", requireAuth, ensureDatabase, async (req, res, next) => {
  try {
    const character = await Character.findOne({ characterId: req.auth.characterId });
    ...
    if (character.characterId !== "street_quintina_quintius") {
      return res.status(403).json({
        error: "Second Take rerolls are only available to the thief.",
      });
    }
    ...
    if (targetCharacterIds.length !== 1) {
      return res.status(400).json({
        error: "Choose one pickpocket target to reroll against.",
      });
    }

    const targets = await getTargets(targetCharacterIds);

    if (targets.length !== 1) {
      return res.status(400).json({
        error: "Selected target is unavailable.",
      });
    }

    const round = await getCurrentRound();
    const remainingBeforeUse = await getSecondTakeRemaining(character.characterId, round);

    if (remainingBeforeUse <= 0) {
      return res.status(400).json({
        error: "No Second Take rerolls remain this round.",
      });
    }

    const resolution = resolvePickpocketRoll({
      character,
      targets,
      isReroll: true,
      remainingRerolls: remainingBeforeUse - 1,
    });
    const cost = normalizeCurrency({});
    const gmBody = gmNoticeBody({
      character,
      ability: { name: "Second Take: Pickpocket Reroll" },
      targets,
      cost,
      roll: resolution.roll,
      rollResults: resolution.targetRolls,
      ...
      resolutionMessage: resolution.message,
      gmExtraLines: [
        `Second Take rerolls remaining after this use: ${remainingBeforeUse - 1}.`,
        ...resolution.gmExtraLines,
      ],
    });
    ...
    await createAndEmitInboxMessage({
      characterId: character.characterId,
      title: "Second Take Used",
      body: `Second Take: ${resolution.message}`,
      type: "ability",
      newState: {
        ability: "Second Take",
        roll: resolution.roll,
        rerollsRemaining: remainingBeforeUse - 1,
      },
    });
